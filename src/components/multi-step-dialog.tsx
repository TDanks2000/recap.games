...
export const MultiStepDialog: FC<MultiStepDialogProps> = ({
	...
	const goToNextStep = useCallback(async () => {
		if (isLastStep || isNavigatingRef.current) return;
		...
		if (handler) {
			...
			try {
				...
				console.error("Error in beforeNext handler:", error);
				return;
			} finally {
				setIsNavigating(false);
				isNavigatingRef.current = false;
	...
	const goToPrevStep = useCallback(() => {
		if (!isFirstStep && !isNavigatingRef.current) {
			goToStep(currentStep - 1);
		}
	...
	return (
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogContent
				className={cn("flex flex-col justify-between", dialogContentClassName)}
			>
